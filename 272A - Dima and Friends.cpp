...
#define wl(a) while(a--)
#define FORCINNM(arr) for(int i=0;i<arr.size();++i){cin>>arr[i];}
...
int main() {
	...
	int numfriends, fingerstotal(0), amt(0);
	cin >> numfriends;
	for (int i = 0,t; i < numfriends; i++) {
		cin >> t;
		fingerstotal += t;
	}
	//0 = dima
	int whichfriend = fingerstotal % ++numfriends;
	for (int i = 1; i <= 5; i++) {
		if ((whichfriend+i)%numfriends-1 != 0) {
			amt++;
		}
	}
	cout << amt;
