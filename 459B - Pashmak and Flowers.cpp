...
int main() {
	...
	cin >> amt;
	veci flowers(amt);
	FORCINNM(amt, flowers);
	auto res = minmax_element(flowers.begin(),flowers.end());
	ull maxdiff = *res.second - *res.first;
	ull amtmin = count(flowers.begin(), flowers.end(), *res.first);
	ull amtmax = count(flowers.begin(), flowers.end(), *res.second);
	sort(flowers.begin(), flowers.end());
	flowers.erase(unique(flowers.begin(), flowers.end()), flowers.end());
	if (flowers.size() != 1) {
		cout << maxdiff << ' ' << amtmin * amtmax;
	}
	else {
		cout << maxdiff << ' ' << amtmin * (amtmin - 1) / 2;
	}
}