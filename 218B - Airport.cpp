...
int main() {
	...
	int numpass, numplanes;
	cin >> numpass >> numplanes;
	veci planes(numplanes);
	FORCINNM(numplanes,planes);
	veci planes2(planes);
	sort(planes.begin(), planes.end());
	int minprice(0), maxprice(0), idxmin(0);
	for (int i = 0; i < numpass; i++) {
		minprice += planes[idxmin];
		planes[idxmin]--;
		if (!planes[idxmin]) {
			idxmin++;
		}
	...
	for (int i = 0; i < numpass; i++) {
		auto maxcost = max_element(planes2.begin(), planes2.end());
		maxprice += *maxcost;
		(*maxcost)--;
	}
	cout << maxprice << ' ' << minprice;
}